...
public:
	...
	Man::Man():rest(true),height(0),velocity(0) {}
	Man::~Man() {}

	void Man::render(ID2D1DeviceContext* m_d2dContext, ID2D1SolidColorBrush* m_pBrush, ID2D1SolidColorBrush* m_pBackgroundBrush, float x, float y, float target) {
...
void VizTheMan::render(
	...
	for(int i = 0; i < MEN_COUNT; i++) {
		...
		((Man**)men)[i]->render(m_d2dContext, m_pBlackBrush, m_pWhiteBrush, 60 + interval * i, 300, offset);
	}
